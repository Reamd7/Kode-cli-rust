...
- [ ] 6.0.2.1 在 `calculate_retry_delay()` 中添加随机 jitter（±10%）
- [ ] 6.0.2.2 使用 `rand` crate 生成随机抖动
- [ ] 6.0.2.3 实现 retry-after header 解析（从响应头读取）
- [ ] 6.0.2.4 优先使用服务器建议的 retry-after 时间
- [ ] 6.0.2.5 添加 retry-after 日志记录
- [ ] 6.0.2.6 编写单元测试验证 jitter 和 retry-after

#### 6.0.3 Session State 缓存 / Session State Caching
- [ ] 6.0.3.1 实现 `set_session_state(key, value)` 函数
- [ ] 6.0.3.2 实现 `get_session_state(key)` 函数
- [ ] 6.0.3.3 实现模型错误的缓存机制
- [ ] 6.0.3.4 在 `try_fix_request()` 中缓存已知的错误类型
- [ ] 6.0.3.5 在 `build_request_body()` 中检查缓存并预应用修复
- [ ] 6.0.3.6 编写单元测试验证缓存逻辑

#### 6.0.4 端点回退机制 / Endpoint Fallback Mechanism
- [ ] 6.0.4.1 实现 `try_with_endpoint_fallback()` 函数
- [ ] 6.0.4.2 支持 MiniMax 端点列表（/text/chatcompletion_v2 → /chat/completions）
- [ ] 6.0.4.3 处理 404 响应并自动尝试下一个端点
- [ ] 6.0.4.4 记录端点切换日志（INFO 级别）
- [ ] 6.0.4.5 支持可配置的端点列表（从配置文件读取）
- [ ] 6.0.4.6 编写单元测试验证回退逻辑

#### 6.0.5 显式代理支持 / Explicit Proxy Support
- [ ] 6.0.5.1 在 `OpenAIConfig` 中添加 `proxy` 字段（可选）
- [ ] 6.0.5.2 使用 `reqwest::Proxy::from_str()` 配置代理
- [ ] 6.0.5.3 支持环境变量 `HTTP_PROXY` 和 `HTTPS_PROXY`
- [ ] 6.0.5.4 添加代理连接日志
- [ ] 6.0.5.5 编写单元测试验证代理配置

#### 6.0.6 工具描述截断 / Tool Description Trimming
- [ ] 6.0.6.1 实现工具描述长度检查（1024 字符限制）
- [ ] 6.0.6.2 实现描述截断逻辑（保留前 1024 字符）
- [ ] 6.0.6.3 将截断部分添加到系统消息中
- [ ] 6.0.6.4 使用 `<additional-tool-usage-instructions>` 格式
- [ ] 6.0.6.5 在 `build_request_body()` 中应用截断
- [ ] 6.0.6.6 编写单元测试验证截断逻辑

#### 6.0.7 调试日志增强 / Debug Logging Enhancement
- [ ] 6.0.7.1 实现 `debug_log_api_call()` 函数（记录 API 调用开始）
- [ ] 6.0.7.2 实现 `log_api_error()` 函数（记录 API 错误详情）
- [ ] 6.0.7.3 记录请求参数（model、max_tokens、temperature 等）
- [ ] 6.0.7.4 记录响应状态码和错误消息
- [ ] 6.0.7.5 使用 `tracing::info!` 和 `tracing::error!`
- [ ] 6.0.7.6 编写集成测试验证日志输出
...
#### 功能完整性 - 80%
- ✅ 核心 API 功能已实现（非流式、流式、工具调用）
- ✅ 参数自适应完整（GPT-5/o1/o3 检测）
- ✅ 错误处理基础完善
- ⚠️ 缺少 AbortSignal 取消支持
- ⚠️ 缺少端点回退机制（MiniMax 等提供商）
- ⚠️ 缺少 session state 缓存
- ⚠️ 重试机制不完整（缺 jitter 和 retry-after）
...
- ✅ 基本性能符合预期
- ✅ 配置格式与 TypeScript 兼容
- ✅ 支持主流提供商（OpenAI、DeepSeek）
- ⚠️ MiniMax 等特殊提供商需要端点回退

### 最终验收标准（100% 对等）

#### 功能完整性 - 100%
- [x] 所有 spec 中的核心需求已实现
- [ ] 高级特性完整实现（AbortSignal、端点回退、session 缓存）
- [ ] 所有场景已测试并通过
- [x] 非流式和流式请求都正常工作
- [x] 工具调用功能完整实现
- [x] 参数自适应正确工作
- [ ] 重试机制完整（jitter、retry-after）
- [ ] 调试日志完善
...
**关键差距：**
1. AbortSignal 支持（用户体验关键）
2. 端点回退（MiniMax 等提供商必需）
3. Session 缓存（性能优化）
4. Jitter 和 retry-after（重试质量）
